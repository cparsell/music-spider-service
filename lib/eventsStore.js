...
import { stringSimilarity } from "./common.js";
 ...
 * the title - as new.
 *
 * A date entry gets `foundAt` (and so can only be `isNew`) if it was
...
// fill in more detail (e.g. "TBA - San Francisco") once it's announced.
// Used to decide whether an existing event's venue should be replaced
// by a more specific value once one arrives - deliberately broad (any
// "TBA*" string will be counted as upgradeable), unlike `isBareTbaVenue`
// below.
function isPlaceholderVenue(venue) {
  return normalize(venue).startsWith("tba");
}

// Only a completely bare "TBA" - no further detail - is a wildcard for
// *grouping* purposes. Two different unannounced shows in the same city
// should not be treated as the same event just because neither has a venue yet.
function isBareTbaVenue(venue) {
  return normalize(venue) === "tba";
}

// Titles count as the same event if one is a substring of the other
// (handles a source dropping/adding a suffix across nights, e.g. "Portola"
// vs "Portola Day 1", which the similarity ratio below scores low on since
// it penalizes length differences rather than rewarding containment) or if
// they're a close-enough edit-distance match (e.g. "Portola Day 1" vs
// "Portola Day 2").
function isSameEventTitle(a, b) {
  ...
  return stringSimilarity(na, nb) >= TITLE_SIMILARITY_THRESHOLD;
}

// The URLs provided by event APIs are specific to each event listing
// (e.g. Ticketmaster's/RA's own per-event ID), so if the incoming result
// shares a URL with any date already stored on this event, it's
// definitely the same event so this skips the title/acts/venue checks.
function hasMatchingUrl(existing, incoming) {
  const incomingUrls = new Set((incoming.urls || []).map((u) => u.url));
  return (existing.dates || []).some((d) =>
    (d.urls || []).some((u) => incomingUrls.has(u.url)),
  );
}

// Two results belong to the same event *series* (e.g. a multi-night run) if
// - they share a source URL (see `hasMatchingUrl`),
// - they have a similar-enough title,
// - share at least one act, AND
// - are at the same venue (or one/both venues are still a bare "TBA" placeholder)
// regardless of which night, so every date gets grouped under one entry instead of
// creating a separate card per night. The title check guards against two
// unrelated shows that are still both unannounced ("TBA") and happen to
// share a single guest act, which title+venue alone can't catch since
// neither has distinguishing venue info yet.
function isSameEventGroup(a, b) {
  if (hasMatchingUrl(a, b)) return true;
  if (!isSameEventTitle(a.eName, b.eName)) return false;
  if (!(a.acts || []).some((act) => (b.acts || []).includes(act))) {
    return false;
  }
  if (isBareTbaVenue(a.venue) || isBareTbaVenue(b.venue)) return true;
...
export async function upsertEvent(event) {
  return withLock(EVENTS_FILE, async () => {
    ...
    if (existing) {
      ...
      if (existingDate) {
        // Deduped on the URL rather than the source name: one show can have
        // several distinct listings from the *same* source (Ticketmaster
        // sells VIP/hospitality packages as separate events alongside the
        // ordinary ticket), and matching on the name kept only one and dropped
        // the rest.
        for (const urlEntry of event.urls || []) {
          if (!existingDate.urls.some((u) => u.url === urlEntry.url)) {
...
/**
 * Same as `setDateCalendarEventId`, but for CalDAV syncs - kept in a
 * separate field since a date can be synced to Google Calendar and a CalDAV
 * calendar independently of each other, and they shouldn't overwrite one
 * another's "already synced" marker.
 */
export async function setDateCalDavEventId(eventId, date, caldavEventId) {
  return withLock(EVENTS_FILE, async () => {
    const events = await readJsonFile(EVENTS_FILE, []);
    const event = events.find((e) => e.id === eventId);
    const dateEntry = event?.dates.find((d) => d.date === date);
    if (dateEntry) dateEntry.caldavEventId = caldavEventId;
    await writeJsonFile(EVENTS_FILE, events);
    return events;
  });
}
