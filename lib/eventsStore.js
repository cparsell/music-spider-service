...
// they have a similar-enough title, share at least one act, and are at the
// same venue (or one/both venues are still a bare "TBA" placeholder) -
// regardless of which night, so every date gets grouped under one entry
// instead of creating a separate card per night. The title check guards
// against two unrelated shows that are still both unannounced ("TBA") and
// happen to share a single guest act, which title+venue alone can't catch
// since neither has distinguishing venue info yet.
