...
/**
 * Returns events with a boolean `isNew` added to each date entry, plus an
 * event-level `isNew` that's true if the event itself is recently created
 * or any of its dates are.
 *
 ...
 * newly-announced extra night on a show we already knew about. The very
 * first date of a brand-new event doesn't get one, since flagging it
 * individually would just be redundant with the event-level badge - see
 * `createdAt` in `upsertEvent`.
 * @param {*} events
 * @returns
 */
export function attachIsNew(events) {
  ...
  return events.map((e) => {
    ...
    const eventRecentlyCreated =
      !!e.createdAt && new Date(e.createdAt).getTime() >= cutoff;
    return {
      ...e,
      dates,
      isNew: eventRecentlyCreated || dates.some((d) => d.isNew),
    };
  });
}
