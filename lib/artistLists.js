...
function createArtistList(filename, defaultList = []) {
  return {
    async getAll() {
      return readJsonFile(filename, defaultList);
    },
    async add(name) {
      const clean = name?.trim();
      if (!clean) throw new Error("Artist name required");
      return withLock(filename, async () => {
        const list = await readJsonFile(filename, defaultList);
    ...
    async remove(name) {
      const clean = name?.trim();
      return withLock(filename, async () => {
        const list = (await readJsonFile(filename, defaultList)).filter(
          (a) => a !== clean,
        );
        await writeJsonFile(filename, list);
        return list;
...
// searches for them would just be noise). "Various Artists" is seeded by
// default - it's a Tautulli/Plex compilation credit, not a real performer,
// so it's noise for basically every install rather than personal taste.
