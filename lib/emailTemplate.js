...
// Matches the "New" pill used on the web UI's event cards/rows.
function newBadge(fontSize) {
  return `<span style="background-color:#ffe066;color:#44494c;font-size:${fontSize}px;font-weight:bold;padding:2px 6px;border-radius:10px;">NEW</span>`;
}
...
function eventCell(event, knownArtists) {
  ...
  cell += `<a href="${primaryUrl}" style="text-decoration:none;"><span style="color:#44494c;font-size:20px;"><strong>${event.eName || ""}</strong></span></a>`;
  if (event.isNew) cell += ` ${newBadge(11)}`;
  cell += `<br/>`;
