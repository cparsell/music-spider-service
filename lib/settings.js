...
const DEFAULT_SETTINGS = {
  ...
  radius: "",
  units: "",
  // Ticketmaster lists VIP/hospitality/premium-seat add-ons as separate
  // events alongside the ordinary ticket for the same show (see
  // isPackageListing in app/api/events/ticketmaster/route.js). Off by
  // default, since for most people they're noise on top of the real listing.
  includeTicketPackages: false,
  ...
  googleClientSecret: "",
  googleRedirectUri: "",
  // Calendar-only alternative to OAuth above - a GCP service account the
  // user shares a calendar with. Takes precedence over OAuth for calendar
  // events when enabled.
  googleServiceAccountEnabled: false,
  googleServiceAccountJson: "", // the key file's JSON, or a path to it
  googleServiceAccountCalendarId: "",
  ...
  // Independent calendar target - works alongside (not instead of) the
  // Google options above, for any CalDAV server (Nextcloud, Radicale,
  // Baikal, iCloud, Fastmail, etc). caldavUrl is the specific calendar
  // collection's URL, not just the server root.
  caldavEnabled: false,
  caldavUrl: "",
  caldavUsername: "",
  caldavPassword: "",
  ...
  smtpHost: "",
  smtpPort: 465,
  smtpSecure: true, // false = STARTTLS on submission port, true = implicit TLS (e.g. port 465)
  smtpUser: "",
  smtpPassword: "",
  smtpFrom: "",
  smtpRecipient: "",
  lastWeeklySmtpEmailSentAt: "",
};
...
const ENV_FALLBACKS = {
  ...
  ticketmasterApiKey: "TICKETMASTER_CONSUMER_KEY",
  latLong: "LAT_LONG",
  radius: "RADIUS",
  units: "UNITS",
  raRegion: "RA_REGION",
  ...
  googleServiceAccountCalendarId: "GOOGLE_SERVICE_ACCOUNT_CALENDAR_ID",
  caldavUrl: "CALDAV_URL",
  caldavUsername: "CALDAV_USERNAME",
  caldavPassword: "CALDAV_PASSWORD",
...
export async function updateSettings(patch) {
  return withLock(SETTINGS_FILE, async () => {
    ...
    // Same idea for the weekly SMTP email.
    if (patch.smtpEnabled && !current.smtpEnabled) {
      updated.lastWeeklySmtpEmailSentAt = new Date().toISOString();
    }
