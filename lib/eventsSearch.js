...
export async function runEventSearch() {
  ...
  try {
    ...
    for (const event of [...raEvents, ...tmEvents]) {
      if (ignoredKeys.has(eventIgnoreKey(event, event.date))) continue;
    ...
    if (config.googleCalendarSyncEnabled && newDates.length > 0) {
      ...
      // has no equivalent in Apps Script or service account mode - there's
      // no user grant to check, so just attempt the sync and let a bad
      // webhook URL/key surface as a calendarError from
      // syncNewDatesToCalendar itself.
      const authorized =
        isServiceAccountEnabled(config) ||
