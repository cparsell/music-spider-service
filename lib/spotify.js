export async function getSpotifyTopArtists(
  ...
  while (results.length < limit) {
    ...
    if (!res.ok) {
      // Spotify error bodies are sometimes JSON ({ error: { message } })
      // and sometimes plain text, depending on the failure type.
      ...
      throw new Error(
        message ? `Spotify API error: ${message} (${res.status})` : `Spotify API error: ${res.status}`,
      );
    }
