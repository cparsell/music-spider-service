...
/**
 * Sorts an list of artists so that known artists are first, preserving the order
 * @param {string[]} acts
 * @param {string[]} known
 * @returns
 */
 ...
 * This function normalizes `knownArtists` to a Set, so callers can pass
 ...
 * lists are shown first, the whole thing capped at `maxCount` entries with
 * a trailing "..." if it gets truncated.
 * @param {string[]} acts
 * @param {Set<string>|string[]} knownArtists
 * @param {number} maxCount
