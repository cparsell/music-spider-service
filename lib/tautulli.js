...
// Tautulli returns HTTP 200 even for API-level failures (e.g. a bad
// apikey), with the real result in `response.result`/`response.message` -
// so a res.ok check alone isn't enough to catch those.
async function fetchHistoryPage(tautulliUrl, params) {
  let res;
  try {
    res = await fetch(`${normalizeBaseUrl(tautulliUrl)}/api/v2?${params}`);
  } catch (err) {
    throw new Error(
      `Could not reach Tautulli at ${normalizeBaseUrl(tautulliUrl)}: ${err.message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Tautulli returned a non-JSON response (${res.status}): ${text.slice(0, 300)}`,
    );
  }

  if (!res.ok || json?.response?.result !== "success") {
    const message = json?.response?.message || `HTTP ${res.status}`;
    throw new Error(`Tautulli API error: ${message}`);
  }
...
export async function getTopArtists(term = "medium_term", count = 200) {
  ...
  while (hasMore) {
    ...
    const rows = await fetchHistoryPage(config.tautulliUrl, params);
